...
const greeting = users.map((user) =>
  user.isPremium === true ? `Bienvenida ${user.name}. Gracias por confiar en nosotros.` : `Bienvenida ${user.name}.`
);
...
const studentWinner = runners
  ...
  .reduce((previous, current) => (previous.time < current.time ? previous : current));
